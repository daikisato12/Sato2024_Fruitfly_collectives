...
#### analysis ####
##### LMM #####
model_speed <- lmerTest::lmer(speed ~ strain * sex * n_inds + 
...
res_single_female <- anova(lm(get(var) ~ strain, data = df_f5min_speed %>% 
...
res_group_female <- anova(lm(get(var) ~ strain, data = df_f5min_speed %>% 
...
res_single_male <- anova(lm(get(var) ~ strain, data = df_f5min_speed %>% 
...
res_group_male <- anova(lm(get(var) ~ strain, data = df_f5min_speed %>% 
...
#### make plot ####
gig_f5min_speed_sex <- df_f5min_speed %>% 
  filter(strain != "norpA") %>%
...
gig_f5min_speed_vs_group <- df_f5min_speed %>%
  filter(strain != "norpA") %>%
...
gig_f5min_speed_vs_sex <- df_f5min_speed %>%
  filter(strain != "norpA") %>%
